...
var MailStation = function(opts) {
	utils.myPrint('MailStation ~ opts = ', JSON.stringify(opts));
...
pro.addServer = function(serverInfo) {
  ...
  this.servers[id] = serverInfo;
	utils.myPrint('this.servers = ', JSON.stringify(this.servers));
	utils.myPrint('this.mailboxes = ', JSON.stringify(this.mailboxes));
	utils.myPrint('Object.keys(this.mailboxes).length = ', Object.keys(this.mailboxes).length);
	utils.myPrint('Object.keys(this.mailboxes) = ', Object.keys(this.mailboxes));
...
pro.addServers = function(serverInfos) {
	utils.myPrint('serverInfos = ', JSON.stringify(serverInfos));
...
pro.dispatch = function(serverId, msg, opts, cb) {
	// console.trace();
	utils.myPrint('serverId = ', serverId);
	utils.myPrint('msg = ', JSON.stringify(msg));
	utils.myPrint('opts = ', opts);
	utils.myPrint('cb = ', cb);
	utils.myPrint('arguments = ', JSON.stringify(arguments));
  if(this.state !== STATE_STARTED) {
...
var lazyConnect = function(station, serverId, factory) {
  ...
  if(!server) {
	...
	utils.myPrint('server = ', JSON.stringify(server));
	utils.myPrint('station.opts = ', JSON.stringify(station.opts));
