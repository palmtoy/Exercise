...
pro.addProxy = function(record) {
  if(!record) {
	...
	utils.myPrint('record = ', JSON.stringify(record));
  var proxy = generateProxy(this, record, this._context);
  if(!proxy) {
    return;
  }

	utils.myPrint('proxy = ', JSON.stringify(proxy));
...
pro.addProxies = function(records) {
	utils.myPrint('records = ', JSON.stringify(records));
	utils.myPrint('records.length = ', records.length);
...
var generateProxy = function(client, record, context) {
  ...
  var res, name;
	...
	utils.myPrint('Client ~ record = ',  JSON.stringify(record));
	utils.myPrint('Client ~ context = ',  JSON.stringify(context));
  var modules = Loader.load(record.path, context);
	utils.myPrint('Client ~ modules = ',  JSON.stringify(modules));
  if(modules) {
    res = {};
    for(name in modules) {
			utils.myPrint('modules[name] = ', Object.keys(modules[name]));
	...
	utils.myPrint('Client ~ res = ',  JSON.stringify(res));
  return res;
};
...
var proxyCB = function(client, serviceName, methodName, args, attach, invoke) {
  ...
  if(args.length < 2) {
	...
	/*
	utils.myPrint('serviceName = ', serviceName);
	utils.myPrint('methodName = ', methodName);
	utils.myPrint('args = ', args);
	utils.myPrint('attach = ', JSON.stringify(attach));
	utils.myPrint('invoke = ', invoke);
	*/

	utils.myPrint('args = ', args);
  var routeParam = args.shift();
	utils.myPrint('typeof args = ', typeof args);
  var cb = args.pop();
  ...
  if(typeof client.router === 'function') {
		utils.myPrint('1 ~ client.router ...');
    route = client.router;
    target = null;
  } else if(typeof client.router.route === 'function') {
		utils.myPrint('2 ~ client.router ...');
	...
	utils.myPrint('route = ', route);
	utils.myPrint('target = ', target);
	utils.myPrint('routeParam = ', routeParam);
	utils.myPrint('_routeContext = ', JSON.stringify(client._routeContext));
